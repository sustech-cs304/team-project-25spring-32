...
//        initialAlbums();  //仅用作展示
    ...
    @NonNull
    private List<Album> getAlbumsByUserId(int id) {
        List<Album> albums = new ArrayList<>();
        //TODO:实现user有关逻辑
        Cursor cursor = albumDao.getAlbumsByUser(id); // 假设用户ID是1，可以改为动态获取
        if (cursor != null) {
            while (cursor.moveToNext()) {
                @SuppressLint("Range") String name = cursor.getString(cursor.getColumnIndex(AlbumDao.COLUMN_NAME));
                @SuppressLint("Range") int userId = cursor.getInt(cursor.getColumnIndex(AlbumDao.COLUMN_USER_ID));
    ...
    public void deleteAlbum(int albumId, String name) {
        ...
        if (result) {
            Log.d("ViewModel", "deleteAlbum: success");
            fileRepository.deleteAlbum(name);
            loadAlbums(); // 重新加载相册列表
